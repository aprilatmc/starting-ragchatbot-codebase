...
class TestRAGSystem:
    ...
    def test_rag_system_initialization(self, mock_config, mock_dependencies):
        ...
        mock_dependencies['tool_manager'].register_tool.assert_any_call(mock_dependencies['search_tool'])
        mock_dependencies['tool_manager'].register_tool.assert_any_call(mock_dependencies['outline_tool'])

    def test_query_without_session_id(self, rag_system, mock_dependencies):
        """Test query method without session ID"""
        # Setup mocks
        mock_dependencies['ai_generator'].generate_response.return_value = "Test AI response"
        mock_dependencies['tool_manager'].get_tool_definitions.return_value = [{"name": "test_tool"}]
        mock_dependencies['tool_manager'].get_last_sources.return_value = [{"text": "Source 1", "link": "http://example.com"}]
        ...
        mock_dependencies['session_manager'].get_conversation_history.assert_not_called()
    ...
    def test_query_with_session_id(self, rag_system, mock_dependencies):
        ...
        # Setup mocks
        mock_dependencies['session_manager'].get_conversation_history.return_value = "Previous conversation context"
        mock_dependencies['ai_generator'].generate_response.return_value = "Contextual AI response"
        ...
        result, sources = rag_system.query("Follow up question", session_id="session123")
        
        # Verify conversation history was retrieved
        mock_dependencies['session_manager'].get_conversation_history.assert_called_once_with("session123")
    ...
    def test_query_tool_usage_flow(self, rag_system, mock_dependencies):
        ...
        mock_dependencies['tool_manager'].get_tool_definitions.return_value = tool_definitions
        mock_dependencies['ai_generator'].generate_response.return_value = "Response based on search results"
        mock_dependencies['tool_manager'].get_last_sources.return_value = [
    ...
    def test_add_course_document_success(self, rag_system, mock_dependencies, sample_course, sample_course_chunks):
        """Test adding a single course document successfully"""
        # Setup mocks
        mock_dependencies['document_processor'].process_course_document.return_value = (sample_course, sample_course_chunks)
        
        course, chunk_count = rag_system.add_course_document("/path/to/course.pdf")
        
        # Verify document processing
        mock_dependencies['document_processor'].process_course_document.assert_called_once_with("/path/to/course.pdf")
        
        # Verify vector store operations
        mock_dependencies['vector_store'].add_course_metadata.assert_called_once_with(sample_course)
        mock_dependencies['vector_store'].add_course_content.assert_called_once_with(sample_course_chunks)
    ...
    def test_add_course_document_error(self, rag_system, mock_dependencies):
        ...
        mock_dependencies['document_processor'].process_course_document.side_effect = Exception("Processing failed")
    ...
    def test_add_course_folder_with_clear_existing(self, rag_system, mock_dependencies, sample_course, sample_course_chunks):
        """Test adding course folder with clear_existing=True"""
        with patch('rag_system.os.path.exists', return_value=True), \
             patch('rag_system.os.listdir', return_value=['course1.pdf', 'course2.txt', 'ignored.doc']):
            
            # Setup mocks
            mock_dependencies['vector_store'].get_existing_course_titles.return_value = []
            mock_dependencies['document_processor'].process_course_document.return_value = (sample_course, sample_course_chunks)
            
            courses_added, chunks_added = rag_system.add_course_folder("/docs", clear_existing=True)
            
            # Verify clear was called
            mock_dependencies['vector_store'].clear_all_data.assert_called_once()
            
            # Verify processing of valid files (2 files: .pdf and .txt)
            assert mock_dependencies['document_processor'].process_course_document.call_count == 2
            assert courses_added == 2
            assert chunks_added == len(sample_course_chunks) * 2

    def test_add_course_folder_skip_existing(self, rag_system, mock_dependencies, sample_course, sample_course_chunks):
        ...
        with patch('rag_system.os.path.exists', return_value=True), \
            ...
            mock_dependencies['vector_store'].get_existing_course_titles.return_value = [sample_course.title]
            mock_dependencies['document_processor'].process_course_document.return_value = (sample_course, sample_course_chunks)
            
            courses_added, chunks_added = rag_system.add_course_folder("/docs")
            
            # Should process but not add existing course
            mock_dependencies['document_processor'].process_course_document.assert_called_once()
            mock_dependencies['vector_store'].add_course_metadata.assert_not_called()
            mock_dependencies['vector_store'].add_course_content.assert_not_called()
    ...
    def test_add_course_folder_nonexistent_directory(self, rag_system, mock_dependencies):
        ...
        with patch('rag_system.os.path.exists', return_value=False):
            ...
            mock_dependencies['document_processor'].process_course_document.assert_not_called()
    ...
    def test_get_course_analytics(self, rag_system, mock_dependencies):
        ...
        mock_dependencies['vector_store'].get_existing_course_titles.return_value = ["Course A", "Course B", "Course C"]
    ...
    def test_query_prompt_formatting(self, rag_system, mock_dependencies):
        ...
        mock_dependencies['ai_generator'].generate_response.return_value = "Test response"
        mock_dependencies['tool_manager'].get_tool_definitions.return_value = []
        mock_dependencies['tool_manager'].get_last_sources.return_value = []
